...
int nearest_frame_vector( struct box *box1, int j1, int j2, int rx, int ry) {
   ...
   if ((!box1->num_frames) || /* JS1904 add output */
       j1<0 || j1>=box1->num_frame_vectors[box1->num_frames-1] ||
       j2<0 || j2>=box1->num_frame_vectors[box1->num_frames-1]) {
...
*/
      ...
      if (box1->m2) {  // what about bad line detection? 2019-04 
        if (sdata->gchar || sdata->hchar) ad=98*ad/100;
      } else              ad=99*ad/100;
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4 (smallest seen is 5x6)
      ...
      if (box1->num_frames != 2) ad=97*ad/100; // excludes tiny holes 1810.rnd80
...
// ..@@@@;..<-
      ...
      // look for no horizontal left gap (~s)
      ...
      if (box1->m2) { // 2019-04 bad line detection ad=96
        if (sdata->gchar || sdata->hchar) ad=99*ad/100;
      } else              ad=99*ad/100;
   ...
   for(ad=d=100;dx>2 && dy>3;){     // Mar2000 perfect??? no ocr-a X Jul09
     ...
     printf("a=%d %d b=%d %d c=%d %d d=%d %d e=%d %d f=%d %d  dxdy %d %d",\
      ...
      if( dx>8 ){ // example szaka0103
        ...
        if( dy<16 && box1->m4!=0 && !hchar ) Break; // test02.pbm
...
static wchar_t ocr0_dD(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      ...
      //if (box1->num_frames != 2) Break; // excludes tiny holes 1810.rnd80
      if (sdata->holes.num < 1) Break;  // JS1904 needed for next [0] access
...
static wchar_t ocr0_pP(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      ...
      if( box1->m4!=0 &&
        (((!hchar) && (!gchar)) || (hchar && gchar))) ad=95*ad/100;
      if( box1->m4==0) { ad=99*ad/100; bc='P'; } else bc='p';
      if( hchar && ((!gchar) || dy<14)) bc='P';
      if (  hchar  &&  gchar) ad=98*ad/100; // \ss sz
      if ((!hchar) && !gchar && box1->m4!=0) ad=98*ad/100; // test02
...
wchar_t ocr0(struct box *box1, pix *bp, int cs){
   ...
   sdata.cs=cs;
   // --- hchar --- gchar -------------------------
   // 2019-04 m1..m4=0 for examples/test03.pbm 4x5-mini-font
   hchar=0;if( box1->m4>0 && y0 < box1->m2-(box1->m2-box1->m1)/2 ) hchar=1;
   gchar=0;if( box1->m4>0 && // 2018-09 nums1 bad m1..m4 
   ...
   */
   ...
   DBG( IFV fprintf(stderr,"\n# holes %d gchar=%d hchar=%d  m= %d %d %d %d",
      sdata.holes.num, gchar, hchar, box1->m1, box1->m2, box1->m3, box1->m4);)
