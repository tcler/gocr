...
Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA  02111-1307, USA.
 ...
 my changes:
   ...
   - **image => *image,  &image[i][1] => &image[i*cols+1] (i*dx+1)
...
/*======================================================================*/
 /* JS 2019-04 changed 2nd+3th argument, new: 2nd=nx=cols, 3th=ny=rows */
int
thresholding (unsigned char *image, int cols, int rows, /* pp dx dy */
  ...
  for (i = y0 /*+ 1*/; i < y0 + dy /*- 1*/; i++) {
    np = &image[i*cols+x0/* +1 JS1904 fix valgrind.memcheck */];
