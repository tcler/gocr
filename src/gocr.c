...
Copyright (C) GPLv2 2000-2019  Joerg Schulenburg
...
static void out_version(int v) {
  fprintf(stderr, " Optical Character Recognition --- gocr "
          ...
          " Copyright (C) 2001-2019 Joerg Schulenburg  GPG=1024D/53BDFBE3\n"
...
static void mark_start(job_t *job) {
  ...
  if (job->cfg.verbose) {
    ...
    /* predefined macros:  gcc -dM -E - </dev/null # __unix __linux */
    /* ... but __DATE__ causes nonreproducable builds */
    fprintf(stderr, "# compiled: " /*__DATE__*/
#if   defined(__GNUC__)  /* can be supported by the author */
     " GNUC "
#elif defined(__TINYC__)  /* can be supported by the author */
     " TINYC "
#else
     " non-GNUC "  /* untested, pleasse report bugs + patches */
#endif
#if defined(__VERSION__)
    __VERSION__  /* gcc +clang */ 
#endif
#if   defined(__linux)
    " linux"     /* supported */
#else
    " non-linux" /* untested, please send bugs + patches */
#endif
       "\n");
    fprintf(stderr,
...
static int read_picture(job_t *job) {
  ...
  if (strstr(job->src.fname, ".pcx"))
    /* readpcx will exit on error (security) */
...
int main(int argn, char *argv[]) {
  ...
  job_init(job); /* init fname, db, cfg */
  ...
  while (multipnm==1) { /* multi-image loop */
    ...
    job_init_image(job); /* single image, ini res.box+line-list, tmp */
